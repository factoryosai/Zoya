...
import { Mic, MicOff, Loader2, Volume2, VolumeX, Keyboard, Send, Trash2, Sun, Brain, Sliders, MessageSquare, Sparkles, Youtube, MessageCircle, Play, Radio, HeartHandshake } from "lucide-react";
...
export default function App() {
  ...
  // Auto-trigger daily affirmation on first load of the day
  useEffect(() => {
    const todayStr = new Date().toDateString();
    const lastAffirmationDate = localStorage.getItem("heer_last_affirmation_date");
    if (lastAffirmationDate !== todayStr) {
      setShowDailyAffirmation(true);
      localStorage.setItem("heer_last_affirmation_date", todayStr);
    }
  }, []);
  ...
  // Hands-Free Wake Word ("Hey Heer") detection
  const handleWakeWordTrigger = useCallback(() => {
    if (!isSessionActive) {
      console.log("Wake Word 'Hey Heer' detected! Connecting session...");
      toggleListening();
    }
  }, [isSessionActive, toggleListening]);

  const { isListeningForWakeWord } = useWakeWord(handleWakeWordTrigger, wakeWordEnabled && !isSessionActive);
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#030712] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <DailyAffirmationModal isOpen={showDailyAffirmation} onClose={() => setShowDailyAffirmation(false)} />
      <MemoryDrawer 
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-4 md:px-8 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-white/5 hover:bg-amber-500/20 hover:text-amber-300 transition-all border border-white/10 text-white/80 flex items-center gap-1.5 text-xs font-mono"
            title="Daily Affirmation & Greeting"
          >
      ...
      <footer className="absolute bottom-0 left-0 w-full flex flex-col items-center justify-center pb-5 md:pb-7 z-20 shrink-0 gap-3">
        ...
        {/* Always-Active Hands-Free Status Indicator */}
        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/60 border border-emerald-500/40 text-emerald-300 text-xs font-mono shadow-lg shadow-emerald-900/30">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
          <span>Hands-Free Listening Active — Just Say <strong className="text-white underline decoration-emerald-400">"Hey Heer"</strong> to Connect!</span>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto max-w-full px-4 scrollbar-hide">
          <button
            onClick={() => setShowDailyAffirmation(true)}
            className="px-3 py-1.5 rounded-full bg-white/5 hover:bg-amber-500/20 hover:text-amber-300 border border-white/10 text-xs font-mono text-white/80 transition-all flex items-center gap-1.5 shrink-0"
          >
            <Sun className="w-3.5 h-3.5 text-amber-300" /> Morning Affirmation
          </button>
